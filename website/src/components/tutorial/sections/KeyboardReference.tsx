      ...
      {lang === 'ko' ? (
        <>
          ...
          <TipBox variant="note">
            ...
            자세한 방법은 위의 <button onClick={() => { document.getElementById('keybinding-custom')?.scrollIntoView({ behavior: 'smooth' }); const base = window.location.hash.split('?')[0]; window.history.replaceState(null, '', `${base}?s=keybinding-custom`); }} className="text-accent-cyan underline underline-offset-2 hover:text-accent-cyan/80">단축키 커스터마이징</button> 섹션을 참고하세요.
          </TipBox>
